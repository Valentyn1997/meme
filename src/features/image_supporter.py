...
sys.path.append(f'{ROOT_PATH}/src/features/')

class ImageCaptioningConverter:
    """ Contains method to convert images to text with the help of image captioning"""

    def __init__(self):

        # Load model
        checkpoint = torch.load(IMAGE_CAPTIONING_WEIGHTS_PATH, map_location=DEVICE)
        ...
        with open(IMAGE_CAPTIONING_WORDMAP_PATH, 'r') as j:
    ...
    def image_to_text(self, img_path):
        ...
        seq, alphas = self._caption_image_beam_search(img_path)
        ...
        sentence.pop(0)  # <start> token
        sentence.pop(-1)  # <end> token
        sentence = ' '.join(sentence)
        return sentence


    def _caption_image_beam_search(self, image_path, beam_size=5):
        """
